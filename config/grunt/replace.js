...
module.exports = (grunt) => {
    return {
        'csp-production': {
            files: {
                'build/web-audio-conference-2016/browser/index.html': ['build/web-audio-conference-2016/browser/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: () => {
                            const html = fs.readFileSync('build/web-audio-conference-2016/browser/index.html', 'utf8'); // eslint-disable-line node/no-sync
                ...
                patterns: [
                    {
                        ...
                        replacement: (_, filename, moduleAttribute) => {
                            if (moduleAttribute === undefined) {
                                return `<script>${fs.readFileSync(`build/web-audio-conference-2016/browser/${filename}`)}</script>`; // eslint-disable-line node/no-sync
                            }

                            return `<script${moduleAttribute}>${fs.readFileSync(`build/web-audio-conference-2016/browser/${filename}`)}</script>`; // eslint-disable-line node/no-sync
                        }
                    }
                ]
            }
